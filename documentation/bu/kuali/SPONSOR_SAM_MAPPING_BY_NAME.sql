--select * from SPONSOR_SAM_MAPPING_BY_NAME order by sponsor_code;
--drop table SPONSOR_SAM_MAPPING_BY_NAME;
--create table SPONSOR_SAM_MAPPING_BY_NAME as 
insert into SPONSOR_SAM_MAPPING_BY_NAME 
(
    samRegistered,
    ueiSAM,
    ueiDUNS,
...
select * from (
    ...
    where 
        upper(rtrim(ltrim(regexp_replace(s.SPONSOR_NAME, '( ){2,}', ' ' )))) = upper(rtrim(ltrim(regexp_replace(u.LEGALBUSINESSNAME, '( ){2,}', ' ' )))) and
        s.DUN_AND_BRADSTREET_NUMBER is null
        and s.SPONSOR_NAME not in (select sponsor_name from dups)
        and not exists (
